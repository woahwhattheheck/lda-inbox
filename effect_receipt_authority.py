...
def _require_live_lease_at(*, expected_document_sha256: str, task_id: str, worker_id: str, lease_id: str, attempt: int, instant: datetime, expected_task_sha256: Optional[str]=None) -> Dict[str,Any]:
    ...
    claimed=_time(execution.get('claimed_at'),'claimed_at'); expires=_time(execution.get('lease_expires_at'),'lease_expires_at')
...
def require_live_lease(*, expected_document_sha256: str, task_id: str, worker_id: str, lease_id: str, attempt: int, now: str, expected_task_sha256: Optional[str]=None) -> Dict[str,Any]:
    return _require_live_lease_at(expected_document_sha256=expected_document_sha256,task_id=task_id,worker_id=worker_id,lease_id=lease_id,attempt=attempt,instant=_time(now,'trusted_time'),expected_task_sha256=expected_task_sha256)

def require_live_lease_current(*, expected_document_sha256: str, task_id: str, worker_id: str, lease_id: str, attempt: int, expected_task_sha256: Optional[str]=None) -> Dict[str,Any]:
    return _require_live_lease_at(expected_document_sha256=expected_document_sha256,task_id=task_id,worker_id=worker_id,lease_id=lease_id,attempt=attempt,instant=_utc_now(),expected_task_sha256=expected_task_sha256)

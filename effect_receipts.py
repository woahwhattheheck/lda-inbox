...
def main(argv:Optional[list[str]]=None)->int:
    ...
    p.add_argument('--attempt',required=True,type=int);_auth_args(p)
    d=sub.add_parser('dispatch');d.add_argument('--token-file',required=True);_auth_args(d)
    ...
    try:
        ...
        if args.command=='dispatch':
            record=read_token_file(Path(args.token_file));_print(ledger.mark_dispatched(task_id=record['task_id'],effect_id=record['effect_id'],token_record=record,expected_document_sha256=args.expected_document_sha256,now=args.now));return 0

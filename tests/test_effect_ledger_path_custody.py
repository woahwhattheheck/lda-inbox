...
class EffectLedgerPathCustodyTests(unittest.TestCase):
    ...
    def _assert_victim_unchanged(self, path: Path, snapshot) -> None:
        ...
        con = sqlite3.connect(path)
        try:
            self.assertEqual(con.execute('SELECT value FROM sentinel').fetchone()[0], 'keep')
            self.assertIsNone(con.execute("SELECT name FROM sqlite_master WHERE name='effects'").fetchone())
        finally:
            con.close()
    ...
    def test_fresh_database_is_created_private_and_usable(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            con = sqlite3.connect(path)
            try:
                self.assertIsNotNone(con.execute("SELECT name FROM sqlite_master WHERE name='effects'").fetchone())
            finally:
                con.close()
